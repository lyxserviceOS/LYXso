...
export async function POST(request: Request) {
  try {
    ...
    const { data: job } = await supabase.from('ppf_jobs').insert({ ...body, org_id: profile.org_id }).select().single();

    return NextResponse.json({ success: true, job, message: 'PPF jobb opprettet' });
  } catch (error) {
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
